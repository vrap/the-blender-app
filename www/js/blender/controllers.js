...
.controller('loginController', [
    ...
    function($scope, $http, $location, $rootScope, $route, UserModel, SessionService, NavService){
        var ip = SessionService.Server.getIP();
        if (null !== ip){
            $location.path('/home');
        }

        $scope.ipAddress = function(isValid, reload){
            $scope.noValid = false;

            if(isValid){
                if(null === ip){
                    SessionService.Server.setIP($scope.ip.ipaddress,$scope.ip.portNumber);
...
.controller('recipeController', [
    ...
    function ($scope, $http, $rootScope, $route, SessionService, ApiService, NavService, RecipeModel){
        ...
        $scope.deleteRecipe = function(cocktailRecipe) {
            var user = SessionService.Users.get();
console.log(user.getCommunity('master').uri);
